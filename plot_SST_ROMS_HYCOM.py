...
for rod in range(0,len(rodada),1):
    filename = "SST_ROMS_GRID_HYCOM_"+rodada[rod]+"_"+data_inicial.strftime("%Y%m%d")+"-"+data_final.strftime("%Y%m%d")+"_"+dominio_str+".mat"
...
for rod in range(0,len(rodada),1):
    ...
    im = ax[rod+1].contourf(lon_roms, lat_roms, locals()['sst_hycom_mean_'+rodada[rod]], np.arange(15,30.001,.5),cmap=cmap, \
