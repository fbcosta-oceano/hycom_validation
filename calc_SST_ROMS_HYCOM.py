...
for rod in range(0,len(rodada),1):
    ...
    sst_rmsd_map = np.ma.array(np.zeros(sst_roms[:,:,0].shape), mask=True)
    sst_rmsd_time = np.ma.array(np.zeros(len(sst_roms[0,0,:])), mask=True)
    ...
    #        if not np.ma.is_masked(sst_roms[lat,lon]):
    #           sst_roms_mean[lat,lon] = remo.nanmean(sst_roms[lat,lon,:])
    #           sst_roms_std[lat,lon] = np.std(sst_roms[lat,lon,:])
    #        if not np.ma.is_masked(sst_hycom[lat,lon]) and not np.ma.is_masked(sst_roms[lat,lon]):
    #           sst_rmsd_map[lat,lon] = remo.rmsd(sst_roms[lat,lon,:],sst_hycom[lat,lon,:])

    sst_hycom_mean = np.mean(sst_hycom,axis=2)
    sst_hycom_mean_time = np.mean(sst_hycom,axis=(0,1))
    sst_hycom_mean_time_diff = np.mean((sst_hycom-sst_roms),axis=(0,1))
    sst_hycom_std = np.std(sst_hycom,axis=2)
    sst_roms_mean = np.mean(sst_roms,axis=2)
    sst_roms_mean_time = np.mean(sst_roms,axis=(0,1))
    sst_roms_std = np.std(sst_roms,axis=2)
    sst_rmsd_map = np.sqrt(np.mean((sst_hycom-sst_roms)**2,axis=2))
    sst_rmsd_time =  np.sqrt(np.mean((sst_hycom-sst_roms)**2,axis=(0,1)))
    #current_data = data_inicial
    #counter = -1
    #while(current_data <= data_final):
    #    counter = counter+1

    #    sst_hycom_temp = np.ma.masked_where(sst_roms[:,:,-1].mask,sst_hycom[:,:,counter])
    #    sst_roms_temp = np.ma.masked_where(sst_hycom[:,:,-1].mask,sst_roms[:,:,counter])
    #    sst_rmsd_time[counter] = remo.rmsd(sst_hycom_temp[:,:].compressed(),sst_roms_temp[:,:].compressed())

    #    del sst_hycom_temp, sst_roms_temp
    #    current_data = current_data + datetime.timedelta(days=inc_tempo)
    ...
    scipy.io.savemat(output_dir+"/"+rodada[rod]+"/"+filename+".mat", mdict={'lat_roms': lat_roms, 'lon_roms': lon_roms, \
                     'sst_hycom_mean_'+rodada[rod]: sst_hycom_mean, 'sst_roms_mean': sst_roms_mean, \
                     'sst_hycom_std_'+rodada[rod]: sst_hycom_std, 'sst_roms_std': sst_roms_std, \
                     'sst_rmsd_time_'+rodada[rod]: sst_rmsd_time, 'sst_rmsd_map_'+rodada[rod]: sst_rmsd_map})

    del sst_roms, sst_hycom_mean, sst_roms_mean, sst_hycom_std, sst_roms_std, sst_rmsd_time
